...
| Install | Pin GitHub `github:thewhatmatters/wmds#<sha>` (CI cannot use `../wmds`). Local path still works. `prepare` builds `dist/`. Current pin: `a80b2b99a7d2ca91bdc46c7fa6b860bf5e0ce42a`. How to consume: WMDS `CONSUMING.md`. After login: WMDS `SegmentedControl` (Insights / Pitch) on `grid-page` + `band` with `--grid-max: 960px`. `AppFrame` mounts WMDS `GridOverlay` (`visibleByDefault`; press **g**) as the first child of `grid-page`. Band children are grid items (`col-span-*`); do not wrap them in `col-span-full flex`. Insights four-up Stats sit on the spine (`col-span-2` / `md:col-span-2` / `lg:col-span-3`) — not nested `Stat.Group`. No AppShell / NavRail / owner rail. Do not invent a Pitchkit Grid atom. |
