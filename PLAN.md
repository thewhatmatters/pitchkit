...
| Install | Pin GitHub `github:thewhatmatters/wmds#<sha>` (CI cannot use `../wmds`). Local path still works. `prepare` builds `dist/`. Current pin: `975b649499da7b54cbc3acbac70dde5e2d9bb915`. How to consume: WMDS `CONSUMING.md`. After login: WMDS `SegmentedControl` (Insights / Pitch) on `grid-page` + `band`. Owner wrap uses Tailwind arbitrary props in JSX — `className="grid-page [--grid-max:960px] [--grid-column-gap:8px] [--grid-gutter:8px] [--grid-cols:12]"` — not React `style` (does not win vs WMDS `@theme`). Force both gap tokens until WMDS `.grid-page` consume path consistently uses `--grid-column-gap` for column-gap. Overlay + band inherit. `AppFrame` mounts WMDS `GridOverlay` (`visibleByDefault`; press **g**) as the first child of `grid-page`. Band children are grid items (`col-span-*`); do not wrap them in `col-span-full flex`. Insights four-up: each Stat sits in a `div` with `col-span-6` / `md:col-span-3` (full class strings in JSX) — not nested `Stat.Group`. No AppShell / NavRail / owner rail. Do not invent a Pitchkit Grid atom. |
...
**Hide from kit:** `media.hidden_from_kit_at` (NULL = on the public kit). Public `/k/[handle]` drops hidden rows **before** `selectSixPosts` (ER + typicals from that visible six). Owner Insights includes every owner row with the timestamp so restore / Undo works. Account `reach_series` / followers do not change. Owner session cookie required: `POST /api/media/hide` and `POST /api/media/restore` `{ mediaId }` → 200 `{ mediaId, hiddenFromKitAt }` (`null` on restore). Idempotent. Errors: `unauthenticated` / `invalid_body` / `not_found` / `forbidden` / `persist_failed`. Seed SoT: in-memory Map (`userId` → mediaId → ISO) on `globalThis` plus httpOnly `pitchkit_hidden` cookie mirror (owner reload after cold start). Public kit reads the Map even without a visitor cookie. Cold start may clear the Map until Neon. No localStorage. Does not delete the media row or R2 object.
...
**Chart series:** Insights kit payload exposes one `reach_series` (`{ day, reach }`, `day` = YYYY-MM-DD UTC). Account reach day buckets (stories + ads). Empty or omit hides the **entire Chart band**. Do not invent 30 zeros. `/insights` reads `owner.reach_series` only. WMDS `Chart.Cartesian` area: `{ date, reach }` points, `animate="none"`, explicit `Area` + `Tooltip` inside a WMDS `Card` (`shape="rounded"` `bodyTerminal` `padding="none"` `col-span-full` — not Storybook `max-w-lg`). `Card.Body` uses the Organisms/Chart **Occupancy history** occupant well (`flex flex-col gap-3` + `cardLayoutBodyOccupantPadYClasses` + `Well` + `InsetX`). Header end is static **30 days** (`cardSubtitleClasses`); no period Select. Single series — no `Chart.Legend`. Host-width paint gate stays **inside** the well so chrome is visible before ink. Owner demo seed includes ~30 labeled example points. Public `/k/demo` omits (Insights missing). Seed only; no Graph poll in this path.
...
| `/insights` | owner cookie | Graph-only Insights: headline Typical reach (hire; hide if missing; `col-span-full`), then a four-up Stat row on the page band — Followers, Engagement rate, Typical reach, Saves when Insights are present; each tile in a `div` with `col-span-6` on mobile and `md:col-span-3` (not nested `Stat.Group`). Do not lead with Engagement rate. Spell **Engagement rate** (never “ER”). No invented period-over-period trends. One 30-day account-reach Chart from `owner.reach_series` when that field is non-empty (Occupancy history Card well in `Card.Body`; static “30 days”; no Select). Hide the whole Chart band when omitted or `[]`. Date ticks use WMDS `chartMaxTicksForWidth` (~3 on a phone). Never zero-fill. **Top-performing posts** with Reach / Engagement / Saves sort; compact single-column rows. Quiet “Private to you” Badge. Top nav is SegmentedControl Insights / Pitch (not AppShell). No contact/past-brands holes. Reconnect / sign out / disconnect stay as buttons. Account is a quiet footer link. |
...
| `/api/media/hide` | owner cookie | `POST { mediaId }` hide from public kit |
| `/api/media/restore` | owner cookie | `POST { mediaId }` restore to public kit |
| `/privacy`, `/delete` | public | Meta review |

Responsive: Insights four-up Stats are band items on a 12-col owner spine — each Stat in a `div` with `col-span-6` (2×2 mobile) and `md:col-span-3` (4 across; do not use `md:col-span-2`). Headline Typical reach wrapper is `col-span-full`. Full `col-span-*` strings live in JSX so Tailwind v4 emits them. Do not nest `Stat.Group` / `gap-4` / `grid-cols-4` on this spine. Insights posts are compact single-column rows. Public kit stays a 2×3 card grid. Owner views wrap `grid-page` + `band` with Tailwind `[--grid-max:960px] [--grid-column-gap:8px] [--grid-gutter:8px] [--grid-cols:12]` and a top SegmentedControl (Insights / Pitch). `AppFrame` paints WMDS `GridOverlay` inside `grid-page`; band children are grid items. A public kit path that skips `AppFrame` has no overlay.
