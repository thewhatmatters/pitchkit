...
Owner home: `/insights`. Pattern header (hug SegmentedControl) toggles Insights vs an in-page shareable-kit preview. Brands only get `/k/[handle]` (shareable Pattern freeze; no owner nav).
...
| `/k/[handle]` | public | Pattern — shareable PitchKit Show code + support footer. PitchKit wordmark only. No owner nav, Edit, MoreMenu, or hide. Share link stays view even with an owner cookie. Hidden posts stay off this card. Owner Edit (contact + past brand names) is on the Insights PitchKit segment. |
