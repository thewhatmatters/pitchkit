...
**Stub vs live OAuth:** same `users` / `media` schema. Stub fills the same columns. Secrets missing → stub + seed `demo`. Testers with `IG_APP_ID` / `IG_APP_SECRET` use Instagram Business Login. Optional operator `IG_USER_TOKEN` polls Graph for **live (non-demo) rows** that have no encrypted OAuth token. Never use it for `DEMO_HANDLE` / seed demo — that bound live Graph identity onto `/k/demo` so Share copied the demo path. Public `/k/demo` stays seed. Do not fork the data model.
