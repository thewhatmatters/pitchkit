...
**Chart series:** Insights kit payload exposes one `reach_series` (`{ day, reach }`, `day` = YYYY-MM-DD UTC). Account reach day buckets (stories + ads). Empty or omit hides the **entire Chart band**. Do not invent 30 zeros. `/insights` reads `owner.reach_series` only. WMDS `Chart.Cartesian` area: `{ date, reach }` points, `animate="none"`, explicit `Area` + `Tooltip` inside an outlined WMDS `Card` (`variant="outlined"` `shape="rounded"` `bodyTerminal` `padding="none"` `col-span-full lg:col-span-6`). `Card.Body` uses the canvas reach well (`flex min-w-0 flex-col gap-4 bg-body px-3.5 py-4` + occupant radius). Header: **Reach over 30 days**. Single series — no invented typical line, no `Chart.Legend`. Host-width paint gate stays **inside** the well. Audience uses `Chart.RankedBars` and hides when mixes are empty. Owner demo seed includes ~30 labeled example reach points plus inventory example mixes so RankedBars can paint (not live Graph). Public `/k/demo` omits Insights. Seed only; no Graph poll in this path.
