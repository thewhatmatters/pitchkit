...
| Install | Pin GitHub `github:thewhatmatters/wmds#<sha>` (CI cannot use `../wmds`). Local path still works. `prepare` builds `dist/`. Current pin: `3f1363082b7b4e591c1690625b9b591bfab5302a` (WMDS #10 — Pattern Show code matches canvas). How to consume: WMDS `CONSUMING.md` and Examples/PitchKit → Pattern — creator Insights (Show code = canvas). After login: copy that shell — `<main className="grid-page min-h-screen bg-body [--grid-column-gap:8px] [--grid-max:1140px] [padding-bottom:44px]">`, header `band pb-4` with three-column brand + default hug `SegmentedControl` + Avatar in a `justify-self-end` span, body `band pt-6 sm:pt-8` / inner `band min-w-0 gap-y-6 sm:gap-y-8`. Paint full-bleed `<body className="bg-body min-h-screen">` in `app/layout.tsx` so WMDS `.grid-page` (max-width + margin auto) is not a gray island on white. No `layout="stretch"`, no `w-full` on the control, no AppFrame `min-h-dvh py-6` replacement surface on Insights or cookie-gated owner kit. Settings / landing / public kit keep `AppFrame` `OWNER_GRID_CLASS` with Tailwind `[--grid-max:1140px] [--grid-column-gap:8px] [--grid-gutter:8px]` — not React `style`. `npm test` fail-closes Pattern shell + owner gap tokens, `hidden_from_kit_at == null` **"N shown"** / rank partition, and Hide-from-kit `AlertDialog` dialog semantics. `GridOverlay` and `ExampleGridControls` stay Storybook-only — not product chrome. One `<Toaster position="bottom-right" />` at `app/layout.tsx`. Band children are grid items (`col-span-*`). Insights four-up: `Stat` `col-span-2 md:col-span-4 lg:col-span-3` — not nested `Stat.Group`. Proof ranking is `Tab.Group`. Hide-from-kit uses `MoreMenu` + `AlertDialog` + `toast.add()` Undo through `hideFromKit` / `restoreToKit`. Owner Recent proof partitions `hidden_from_kit_at` on init and when `posts` change — **"N shown"** and rank use `null` only; hidden rows stay with muted Badge + **Restore to kit**. No AppShell / NavRail / owner rail. Do not invent a Pitchkit Grid atom. |
...
Owner home: `/insights`. Pattern header (hug SegmentedControl PitchKit) is owner chrome over the same card at `/k/[handle]`. Brands only get `/k/[handle]` (no owner nav).
...
**Chart series:** Insights kit payload exposes one `reach_series` (`{ day, reach }`, `day` = YYYY-MM-DD UTC). Account reach day buckets (stories + ads). Empty or omit hides the **entire Chart band**. Do not invent 30 zeros. `/insights` reads `owner.reach_series` only. WMDS `Chart.Cartesian` + `Chart.Legend` inside an outlined WMDS `Card` (`variant="outlined"` `shape="rounded"` `bodyTerminal` `col-span-full lg:col-span-6`). `Card.Body` uses the canvas reach well (`flex min-w-0 flex-col gap-4 bg-body px-3.5 py-4` + occupant radius). Header: **Reach over 30 days** + supporting copy + **Graph data** Badge. Dual series matches Pattern: daily `reach` from `reach_series` plus a constant Typical reach reference from the existing `typicalReach` median — not a second Graph time series, not Stat `trend`s. `minHeight={344}`. `animate="none"`. Audience uses `Chart.RankedBars` and hides when mixes are empty. Owner demo seed includes ~30 labeled example reach points plus inventory example mixes so RankedBars can paint (not live Graph). Public `/k/demo` omits Insights. Seed only; no Graph poll in this path.
...
Responsive: Insights four-up Stats sit on the WMDS page subgrid — each `Stat` uses `col-span-2 md:col-span-4 lg:col-span-3`. Do not nest `Stat.Group`. Recent proof cards use `md:col-span-4 lg:col-span-4`. Public kit stays a 2×3 card grid. Cookie-gated Insights / owner kit copy canvas bands (`band pb-4` + `band pt-6 sm:pt-8` / inner `gap-y-6 sm:gap-y-8`). Settings / public `AppFrame` still uses Tailwind `[--grid-max:1140px] [--grid-column-gap:8px] [--grid-gutter:8px]`. `GridOverlay` is Storybook-only.
