...
| Install | Pin GitHub `github:thewhatmatters/wmds#<sha>` (CI cannot use `../wmds`). Local path still works. `prepare` builds `dist/`. Current pin: `73277bab5bd3ffc8dff678c12d4cdbc415b07a35` (WMDS Tooltip + scroll fade available; Pitchkit still Insights + toast). How to consume: WMDS `CONSUMING.md` and Examples/PitchKit → Pattern — creator Insights. After login: WMDS `SegmentedControl` (Insights / PitchKit) on `grid-page` + `band`. Owner wrap uses Tailwind arbitrary props in JSX — `OWNER_GRID_CLASS` / `className="grid-page [--grid-max:1140px] [--grid-column-gap:8px] [--grid-gutter:8px]"` — not React `style`. Force both gap tokens until WMDS `.grid-page` consume path consistently uses `--grid-column-gap` for column-gap. `npm test` fail-closes owner gap tokens, `hidden_from_kit_at == null` **"N shown"** / rank partition, and Hide-from-kit `AlertDialog` dialog semantics. `GridOverlay` and `ExampleGridControls` stay Storybook-only — not product chrome. One `<Toaster position="bottom-right" />` at `app/layout.tsx`. Band children are grid items (`col-span-*`). Insights four-up: `Stat` `col-span-2 md:col-span-4 lg:col-span-3` — not nested `Stat.Group`. Proof ranking is `Tab.Group`. Hide-from-kit uses `MoreMenu` + `AlertDialog` + `toast.add()` Undo through `hideFromKit` / `restoreToKit`. Owner Recent proof partitions `hidden_from_kit_at` on init and when `posts` change — **"N shown"** and rank use `null` only; hidden rows stay with muted Badge + **Restore to kit**. No AppShell / NavRail / owner rail. Do not invent a Pitchkit Grid atom. |
