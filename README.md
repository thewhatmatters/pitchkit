...
Until Hyperdrive exists, `/k/demo` and `/insights` read the in-repo seed (`lib/seed.ts`). Same `User` / `Media` types as live. `TOKEN_KEY` is not required for seed. Unknown handle (`/k/nope`) is 404. No Neon or Instagram token yet. Hide/restore on the seed path writes an httpOnly `pitchkit_hidden` overlay keyed to the session user (reload keeps hide in that browser). When Hyperdrive exists, write `media.hidden_from_kit_at` instead.
