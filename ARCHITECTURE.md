...
| Hyperdrive → Neon | `users`, `media` (including `hidden_from_kit_at`), empty `detections` and `weekly_counts`. Bindings: `HYPERDRIVE` / `HYPERDRIVE_PREVIEW`. Until Hyperdrive exists, `/k/demo` and `/insights` read `lib/seed.ts` — same types as live. Hide/restore persist in httpOnly `pitchkit_hidden` (session-user keyed). Schema SQL: `db/*.sql`. |
