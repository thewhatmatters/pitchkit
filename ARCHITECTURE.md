...
- **Seed:** In-repo rows match [DATA.md](./DATA.md). `TOKEN_KEY` not required (seed tokens are null). Disconnect columns exist; no live delete yet. Public `/k/demo` has no Insights (`reach_series` omitted). Owner Insights seed includes example `reach_series` (not a SQL table, not live Graph). `hidden_from_kit_at` seed default is null. Hide/restore seed SoT is an in-memory Map + `pitchkit_hidden` cookie mirror; cold start may clear the Map until Neon.
...
| Piece | Role |
|---|---|
| Workers / OpenNext | All HTML and APIs. Sets the httpOnly session cookie. Encrypts tokens with `TOKEN_KEY` before SQL. Until live OAuth, stub `/auth/instagram` sets `pitchkit_session` for seed `demo` (not a token). `/auth/sign-out` clears it and `pitchkit_hidden`. Owner hide/restore: `POST /api/media/hide` and `POST /api/media/restore`. |
...
| Hyperdrive → Neon | `users`, `media` (including `hidden_from_kit_at`), empty `detections` and `weekly_counts`. Bindings: `HYPERDRIVE` / `HYPERDRIVE_PREVIEW`. Until Hyperdrive exists, `/k/demo` and `/insights` read `lib/seed.ts` — same types as live. Hide/restore seed SoT is an in-memory Map (`globalThis`) plus httpOnly `pitchkit_hidden` (cookie = owner reload after cold start). Schema SQL: `db/*.sql`. |
