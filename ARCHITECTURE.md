...
- **Install WMDS:** pin `github:thewhatmatters/wmds#a80b2b99a7d2ca91bdc46c7fa6b860bf5e0ce42a` (CI cannot use `../wmds`). Local `../wmds` still works; `prepare` builds `dist/`. `@visx/visx` is the Chart peer. Owner views: `grid-page` + `band` with `--grid-max: 960px` and WMDS `SegmentedControl` (Insights / Pitch). `AppFrame` mounts WMDS `GridOverlay` (`visibleByDefault`; press **g**) as a direct child of `grid-page`. Band children are grid items — Insights four-up Stats use `lg:col-span-3` (3 tracks each on 12-col). Owner nav, private badge, Chart, posts, and footer use `col-span-full`. No AppShell / NavRail. Do not invent a Pitchkit Grid atom.
