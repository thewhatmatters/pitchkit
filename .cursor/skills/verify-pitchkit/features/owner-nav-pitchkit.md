...
- **Open PitchKit.** Choose PitchKit. Run `node .cursor/skills/verify-pitchkit/helpers/control-pitchkit.mjs click --role radio --name "PitchKit"`. Path stays `/insights`. The shareable kit shows **Demo Creator**, `@demo`, Followers, Engagement rate, and **Selected posts**. Owner **Edit** switch may be present; Insights PageHeader / Recent proof / Share kit are absent on this segment.
...
- Share link / a brand opening `/k/demo` must stay the public freeze — do not treat owner Edit as the public kit.
