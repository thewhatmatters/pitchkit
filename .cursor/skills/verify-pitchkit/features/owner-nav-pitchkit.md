...
- **Open PitchKit.** Choose PitchKit. Run `node .cursor/skills/verify-pitchkit/helpers/control-pitchkit.mjs click --role radio --name "PitchKit"`. Path stays `/insights`. Body shows the owner kit: PageHeader **PitchKit** + primary **Share kit**, **Demo Creator**, `@demo`, **Followers**, **Engagement rate**, **Selected posts**, and MoreMenu **Manage selected post N** / **Hide from kit**. No **Coming soon** badge. No bio / website / rates / Edit switch. Insights PageHeader / Recent proof are absent on this segment. Share kit is the owner PitchKit primary copy action; Insights keeps Pattern Share kit.
