...
- **Gap tokens.** Run `node .cursor/skills/verify-pitchkit/helpers/control-pitchkit.mjs eval --js "getComputedStyle(document.querySelector('.grid-page')).getPropertyValue('--grid-max').trim()"` (expect `1140px`) and the same for `--grid-column-gap` (expect `8px`). Pattern Show code does not set `--grid-gutter` on this `<main>`.
