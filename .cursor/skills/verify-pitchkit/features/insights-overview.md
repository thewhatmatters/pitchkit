...
- `insights-insufficient-audience` keeps the Audience Card with **No audience data yet** / **Connect Instagram Insights demographics when available.** when mixes are empty after Graph or on the tokenless seed (not Skeleton, not Chart.Loading, not EXAMPLE %, not omit-card).
...
- **Open Insights.** Run `node .cursor/skills/verify-pitchkit/helpers/control-pitchkit.mjs goto /insights`. Path is `/insights`. Heading **Insights**. Copy includes **Private to you**. The Engagement rate formula is not a PageHeader headline — it lives on the **Engagement rate** Stat info tooltip (`Engagement rate = (likes + comments + saves + shares) ÷ reach`).
- **Four-up Stats.** The group `Instagram performance summary` lists **Followers**, **Engagement rate**, **Typical reach**, and **Saves**. The Engagement rate tile has an info control named **Engagement rate formula**. Seed complete-kit values: Followers `10,000`, Engagement rate `9.5%`, Typical reach `2,175`, Saves `42`. Live may already have a Hidden proof post (`N shown` < 6); typicals stay on the six-or-fewer still shown. Still require the four labels and no “ER”.
...
- **Audience.** When mixes exist, **Audience fit** is visible with RankedBars named **Audience by countries**, **Audience by cities**, **Audience by age**, and **Audience by gender**. If every mix is empty, the Card stays with **No audience data yet** / **Connect Instagram Insights demographics when available.** (`[data-audience-slot="empty"]`) — no EXAMPLE country/city/age/gender percents, no zero bars. A single empty mix hides that section only. Reach can stay resolved while Audience is empty. Both Cards stay when both Graph series are insufficient.
