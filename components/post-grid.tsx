...
export function PostGrid({ posts, hasInsights, loading = false }: PostGridProps) {
  if (loading) {
    return (
          ...
          <Card.Body>
            ...
            <div className="px-3 py-2">
              <p className={cardBodyTextClasses}>
                ...
                {hasInsights && post.reach != null ? ` · ${formatCount(post.reach)} reach` : ""}
                {hasInsights && post.saves != null ? ` · ${formatCount(post.saves)} saves` : ""}
              </p>
            </div>
          </Card.Body>
        </Card>
