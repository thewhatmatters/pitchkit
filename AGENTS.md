...
- **UI:** `@whatmatters/wmds` (pattern-first, import `styles.css`). Layout Tailwind only (`grid-page` + `band` from CONSUMING). Owner views: `SegmentedControl` + `--grid-max: 960px`. Do not compose AppShell, NavRail, or a replacement shell. Lucide via WMDS. No shadcn. No Storybook in this repo. Pin `github:thewhatmatters/wmds#<sha>` (CI cannot use `../wmds`); `prepare` builds `dist/`. Current pin: `a80b2b99a7d2ca91bdc46c7fa6b860bf5e0ce42a` (2026-09-07 main, AppShell stripped). After install, copy Geist into `node_modules/@whatmatters/wmds/dist/files` (`postinstall` / `predev` / `prebuild`).
