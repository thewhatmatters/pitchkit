...
- After login: **Insights**, then Media kit tab. Brands never see Insights. `/insights` this round is a stacked **example** inventory of locked kit objects for Design (not a layout lock). Banner: “Example data — not live.” `/k/demo` stays the public kit card.
...
Do not add Graph columns we do not get from public posts + Insights. Do not invent rules that contradict [PLAN.md](./PLAN.md). Do not invent Stat, Chart, or bar atoms here — Card / text / Chip only. Chart slot is a labeled empty box until WMDS Chart ships. Do not paint rates, stories, impressions, hometown, industry, profile views, or bio-link clicks.
