...
- Hide from kit: `media.hidden_from_kit_at`. Public kit excludes hidden **before** `selectSixPosts`. Owner Insights includes all rows with the timestamp. `POST /api/media/hide` and `/api/media/restore` `{ mediaId }`, owner session only. Seed persist: httpOnly `pitchkit_hidden` until Hyperdrive. No localStorage.
