...
- **UI:** `@whatmatters/wmds` (pattern-first, import `styles.css`). Layout Tailwind only (`grid-page` + `band` from CONSUMING). Cookie-gated Insights copies Pattern — creator Insights Show code (hug `SegmentedControl` in a three-column header; PitchKit segment previews the shareable kit). Public `/k/[handle]` copies Pattern — shareable PitchKit. Settings / landing `AppFrame` still uses Tailwind `[--grid-max:1140px] [--grid-column-gap:8px] [--grid-gutter:8px]`. Do not mount `GridOverlay` in product chrome. Do not invent a Pitchkit Grid atom. Do not compose AppShell. Lucide via WMDS. No shadcn. No Storybook in this repo. Pin `github:thewhatmatters/wmds#75f8a41e8b131906378b340a4106a486ddd5173f`. After install, copy Geist into `node_modules/@whatmatters/wmds/dist/files`.
