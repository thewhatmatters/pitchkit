...
- After login: **Insights**. Chrome copies WMDS Pattern — creator Insights Show code (`examples-pitchkit--creator-insights`): `<main className="grid-page min-h-screen bg-body [--grid-column-gap:8px] [--grid-max:1140px] [padding-bottom:44px]">`, header `band pb-4` with three-column `grid-cols-[1fr_auto_1fr]` (PitchKit label + default hug `SegmentedControl` + `Avatar` in `justify-self-end`), body `band pt-6 sm:pt-8` / inner `band min-w-0 gap-y-6 sm:gap-y-8`. PitchKit segment is an in-page **Coming soon** placeholder (WMDS `PitchKitPlaceholder` copy) so the selected-pill can move — do not hard-nav `/insights` ↔ `/k/…` and do not mount `KitEdit` there. Full-bleed `<body className="bg-body min-h-screen">`. No `layout="stretch"`, no `w-full` on the control, no AppFrame `min-h-dvh py-6` surface on Insights or the public kit. `GridOverlay` and `ExampleGridControls` stay Storybook-only. One `<Toaster position="bottom-right" />` at `app/layout.tsx`. Insights body matches the Pattern: `PageHeader`, four-up `Stat` (`col-span-2 md:col-span-4 lg:col-span-3`), outlined `Chart.Cartesian` + `Chart.RankedBars`, **Recent proof** via `Tab.Group`, `MoreMenu` + `AlertDialog` hide + `toast.add()` Undo through `hideFromKit(mediaId)` / `restoreToKit(mediaId)` (`POST /api/media/hide` + `POST /api/media/restore`). Spell **Engagement rate** (never “ER”). No invented trends. Hide empty chart series and empty audience mixes. No AppShell. Account is a quiet footer link to `/settings`. Brands never see Insights. Public `/k/demo` omits `reach_series`. Owner demo includes seed/example Insights + `reach_series`.  Settings / landing stay on `AppFrame` (`OWNER_GRID_CLASS` still forces `[--grid-gutter:8px]`).
- `/k/[handle]` (and `/k/demo`) is the public kit — Pattern — shareable PitchKit Show code (`examples-pitchkit--shareable-pitchkit`): PitchKit wordmark, identity + Instagram Chip, followers / Engagement rate Stats, selected posts (likes/comments only), Contact `TextLink` when sourced, Past brands Cards when sourced. No Edit / MoreMenu / hide / swap / Coming soon. Share link stays view even with an owner cookie. Owner PitchKit segment on `/insights` is Coming soon, not a kit preview. Hide empty contact and brands; no marquee; do not invent Verified, website, location, or brand year/summary. Settings (`/settings`) is account only.
- Six posts: last 30 days, **saves then reach then likes**. Engagement rate: `(likes + comments + saves + shares) / reach` (sum÷sum on posts with Insights `reach` > 0; unlocked 2026-09-18). Hide (or —) when reach is missing — do not fall back to ÷ followers. If Insights missing, hide Engagement rate, reach, saves, and the chart. Spell **Engagement rate**.
...
- **UI:** `@whatmatters/wmds` (pattern-first, import `styles.css`). Layout Tailwind only (`grid-page` + `band` from CONSUMING). Cookie-gated Insights copies Pattern — creator Insights Show code (hug `SegmentedControl` in a three-column header; PitchKit segment is Coming soon). Public `/k/[handle]` copies Pattern — shareable PitchKit. Settings / landing `AppFrame` still uses Tailwind `[--grid-max:1140px] [--grid-column-gap:8px] [--grid-gutter:8px]`. Do not mount `GridOverlay` in product chrome. Do not invent a Pitchkit Grid atom. Do not compose AppShell. Lucide via WMDS. No shadcn. No Storybook in this repo. Pin `github:thewhatmatters/wmds#75f8a41e8b131906378b340a4106a486ddd5173f`. After install, copy Geist into `node_modules/@whatmatters/wmds/dist/files`.
