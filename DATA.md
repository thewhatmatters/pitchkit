...
- emails
- industry / category
- bio, website, rates, “contact for collab,” geo, email (not SQL columns. Pitchkit-owned `intro` and `past_brands` live on the KV Graph snapshot / kit payload — not Instagram `biography`, not Hyperdrive columns this cut.)
...
Owner demo seed (`loadOwnerKit`) includes ~30 labeled example points and never uses operator `IG_USER_TOKEN` or a Graph snapshot for media (handle stays `demo`). A live OAuth token polls Graph and persists users/media (SQL when Hyperdrive is bound; else KV `graph:` keys). `reach_series` + audience stay payload extras (`graph:payload:` when SQL owns the rows) — not a SQL table. **Intro + past brands + theme** are the same payload family: `intro: string | null` (Pitchkit-owned; soft 160 / hard 280), `past_brands: { id: string; name: string; logo_key?: string | null; result_label?: string | null }[]` (max 8; array order; unknown `logo_key` → letter Avatar; empty `result_label` omitted; strip `result_label` over 24), and `theme: "light" | "dark" | "soft"` (default `light`). Hyperdrive stays parked — no SQL columns / no Hyperdrive rebind. `isSnapshot` stays tolerant of older blobs missing those keys. Graph poll / OAuth rewrite preserves them. Public `/k/demo` (`loadPublicKit`) stays seed media and omits `reach_series` (insufficient-reach empty well); seed ships frozen intro/brands and `theme: light` until a demo snapshot overrides. Live public kits receive `reach_series` + audience from the Graph snapshot so the shareable Pattern can paint compact 30-day reach and top 3 countries. Live empty lists omit those blocks. Frontend: one WMDS Chart on `/insights` from `owner.reach_series` (full-height); public `/k/[handle]` paints the compact Reach Card (empty well when the series cannot be plotted — never omit, never zeros). No period-over-period KPI deltas in the payload — do not invent Stat `trend`s.
