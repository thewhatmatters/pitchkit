...
Owner demo seed (`loadOwnerKit`) includes ~30 labeled example points and never uses operator `IG_USER_TOKEN` or a Graph snapshot for media (handle stays `demo`). A live OAuth token polls Graph and persists users/media (SQL when Hyperdrive is bound; else KV `graph:` keys). `reach_series` + audience stay payload extras (`graph:payload:` when SQL owns the rows) — not a SQL table. **Intro + past brands** are the same payload family: `intro: string | null` (Pitchkit-owned; soft 160 / hard 280) and `past_brands: { id: string; name: string }[]` (max 8; array order). Hyperdrive stays parked — no SQL columns / no Hyperdrive rebind. `isSnapshot` stays tolerant of older blobs missing those keys. Graph poll / OAuth rewrite preserves them. Public `/k/demo` (`loadPublicKit`) stays seed media and omits `reach_series`; seed ships frozen intro/brands until a demo snapshot overrides. Live empty lists omit the public blocks. Frontend: one WMDS Chart on `/insights` from `owner.reach_series` only; keep the Reach Card empty band when Insights exist but the series is unusable; omit when Graph never returned Insights. Never zero-fill. Public kit never paints the Chart. No period-over-period KPI deltas in the payload — do not invent Stat `trend`s.
